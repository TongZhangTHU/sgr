...
def fill_replay(cfg: DictConfig,
    ...
    variation_numbers = variation_number if isinstance(variation_number, ListConfig) or isinstance(variation_number, list) else [variation_number]
    for variation_number in variation_numbers:
        for d_idx in range(num_demos):
            ...
            if simulation:
                demo = rlbench_utils.get_stored_demos(
                    ...
                    variation_number=variation_number, task_name=task,
            ...
            if rank == 0:
                logging.info(f"Loading Demo({d_idx}) - found {len(episode_keypoints)} keypoints - {task} - {variation_number}")
        ...
        logging.debug('Replay %s variation % s filled with demos.' % (task, variation_number))
...
def fill_multi_task_replay(cfg: DictConfig,
    ...
    for split in split_n:
        for e_idx, task_idx in enumerate(split):
            ...
            model_device = torch.device('cuda:%s' % (rank % torch.cuda.device_count())
                                        if torch.cuda.is_available() else 'cpu')
