...
class CustomRLBenchEnv(RLBenchEnv):
    ...
    def reset(self) -> dict:
        self._i = 0
        self._task.set_variation(self._variation_number)
...
class CustomMultiVariationRLBenchEnv(RLBenchEnv):
    ...
    def __init__(self,
                 ...
                 include_lang_goal_in_obs: bool = False,
                 record_every_n: int = 20,
                 variation_numbers: List[int] = [-1]
                 ):
        super(CustomMultiVariationRLBenchEnv, self).__init__(
            task_class, observation_config, action_mode, dataset_root,
            channels_last, headless=headless,
            include_lang_goal_in_obs=include_lang_goal_in_obs)
        self._reward_scale = reward_scale
        ...
        self._last_exception = None
        self._variation_numbers = variation_numbers

        self._swap_variation_every = swap_variation_every
        self._episodes_this_task = 0 # self._episodes_this_task is the same as self._episode_index ?

    @property
    def observation_elements(self) -> List[ObservationElement]:
        obs_elems = super(CustomMultiVariationRLBenchEnv, self).observation_elements
        for oe in obs_elems:
            if oe.name == 'low_dim_state':
                oe.shape = (oe.shape[0] - 7 * 3 + int(self._time_in_state),)  # remove pose and joint velocities as they will not be included
                self.low_dim_state_len = oe.shape[0]
    ...
    def extract_obs(self, obs: Observation, t=None, prev_action=None):
        ...
        obs_dict = super(CustomMultiVariationRLBenchEnv, self).extract_obs(obs)
    ...
    def launch(self):
        super(CustomMultiVariationRLBenchEnv, self).launch()
        self._task._scene.register_step_callback(self._my_callback)
        if self.eval:
            cam_placeholder = Dummy('cam_cinematic_placeholder')
            cam_base = Dummy('cam_cinematic_base')
    ...
    def reset(self) -> dict:
        ...
        self._variation_number_now = self._variation_numbers[(self._episodes_this_task // self._swap_variation_every) % len(self._variation_numbers)]
        self._episodes_this_task += 1

        self._task.set_variation(self._variation_number_now)
        self._previous_obs_dict = super(CustomMultiVariationRLBenchEnv, self).reset()
        self._record_current_episode = (
                self.eval and self._episode_index % self._record_every_n == 0)
        self._episode_index += 1
        self._recorded_images.clear()
