...
PTH_PATH ={
    'moco': '/path/to/moco', # https://dl.fbaipublicfiles.com/moco/moco_checkpoints/moco_v2_800ep/moco_v2_800ep_pretrain.pth.tar
    'vicregl': '/path/to/vicregl', # https://dl.fbaipublicfiles.com/vicregl/resnet50_alpha0.9.pth
    'simsiam': '/path/to/simsiam', # https://dl.fbaipublicfiles.com/simsiam/models/100ep-256bs/pretrain/checkpoint_0099.pth.tar
    'r3m': '/path/to/r3m', # https://drive.google.com/uc?id=1Xu0ssuG0N1zjZS54wmWzJ7-nb0-7XzbA
    'ulip': '/path/to/ulip' # https://storage.cloud.google.com/sfr-ulip-code-release-research/pretrained_models/ckpt_zero-sho_classification/checkpoint_pointnext.pt?authuser=0
}
...
def quaternion_to_soft_discrete_euler(quaternion, resolution):
    euler = Rotation.from_quat(quaternion).as_euler('xyz', degrees=True) + 180
    assert np.min(euler) >= 0 and np.max(euler) <= 360
    # disc = np.around((euler / resolution)).astype(int)
    disc = euler / resolution
    disc[disc == int(360 / resolution)] = 0
    return disc
...
def point_to_voxel_index_no_clip(
        point: np.ndarray,
        voxel_size: np.ndarray,
        coord_bounds: np.ndarray):
    # diff 1: remove upper bound (dims_m_one) clipping
    # diff 2: remove res + 1e-12
    bb_mins = np.array(coord_bounds[0:3])
    bb_maxs = np.array(coord_bounds[3:])
    # dims_m_one = np.array([voxel_size] * 3) - 1
    bb_ranges = bb_maxs - bb_mins
    res = bb_ranges / np.array([voxel_size] * 3)
    # voxel_indicy = np.minimum(
    #     np.floor((point - bb_mins) / (res + 1e-12)).astype(
    #         np.int32), dims_m_one)
    voxel_indicy = np.floor((point - bb_mins) / res).astype(np.int32)
    return voxel_indicy
...
def extract_obs(obs: Observation,
    ...
    if not channels_last:
        ...
        #         # here front_rgb uint8 (3, 128, 128) 105.95353190104167
        #         # front_point_cloud float64 (3, 128, 128) 0.11006913327501248
        #         print(k)
        #         print(v.dtype, v.shape, v.mean())
        #         print(obs_dict[k].dtype, obs_dict[k].shape, obs_dict[k].mean())
    else:
...
def setup_logger(file_name='train.log', logging_level=20):
