# Adapted from https://github.com/peract/peract/blob/main/voxel/augmentation.py
import numpy as np
import torch
...
def perturb_se3(pcd,
    ...
    for p in pcd:
        ...
        # bound gripper pose, not pcd, pcd can be out of bounds
...
def apply_se3_augmentation(pcd,
    ...
    :param layer: voxelization layer (always 0 for non-coarse-to-fine models, such as SGR, SGRv2, and PerAct) 
    ...
    perturb_attempts = 0
    # compared with peract, add torch.any(perturbed_trans > voxel_size - 1) 
    while torch.any(perturbed_trans < 0) or torch.any(perturbed_trans > voxel_size - 1):
        # might take some repeated attempts to find a perturbation that doesn't go out of bounds
        perturb_attempts += 1
        if perturb_attempts > 10000:
        ...
        # add trans_continuous and quats outputs
        trans_continuous = []
        quats = []
        for b in range(bs):
            ...
            # modify point_to_voxel_index to no clip for out of bounds check
            trans_idx = utils.point_to_voxel_index_no_clip(perturbed_action_trans[b], voxel_size, bounds_np)
            trans_indicies.append(trans_idx.tolist())
            trans_continuous.append(perturbed_action_trans[b].tolist())
        ...
        perturbed_trans_continuous =  torch.from_numpy(np.array(trans_continuous)).to(device=device)
        perturbed_rot_grip = torch.from_numpy(np.array(rot_grip_indicies)).to(device=device)
        quats = torch.from_numpy(np.array(quats)).to(device=device)

    action_trans = perturbed_trans
    action_trans_continuous = perturbed_trans_continuous
    ...
    return action_trans, action_rot_grip, pcd, action_trans_continuous.float(), quats